...
function draw_keyword_panel( kid ) {
	...
	var panel = d3.select('#top-row').append("div").attr("class", "panel panel-info keypanel")
					.attr("id", "keyword"+kid)
					.style("left", (window.screen.width * 0.69 - num_keywords * 100 )+ "px")
...
function generate_weibo_in_panel(weibolist, panel_body, kid ) {
	...
	var lists = panel_body.append("div").attr("class", "panel_weibo")
			.attr("style", "max-height: 400px; overflow: scroll")
			.attr("id", "keyword"+kid)
	...
	lists.attr("id","list"+kid).append('span').style("font-weight", "bold").text( function (d) { 
			return weibos[d]["user"] + " [" + Timestamp(weibos[d]['time'])+ "] " ;} );
	lists.selectAll('span').data( function(d) { return weibotext_to_array(weibos[d], kid);} )
		.enter().append("span").attr("style", function(d) {return d["style"]})
...
function weibotext_to_array( weibo, kid ) {
	//return [weibo['text']];
	...
	for (var i = 0;i < post.length;++i) {
		if (i > 0 && post[i] < post[pre]+keywords[key[pre]]['word'].length)  
			continue;
		if (i == 0) {
			ret[j++] = {'text':text.substring(0, weibo['post'][i]), "style": "color: none" };
		}else {
			ret[j++] = {'text':text.substring(post[pre]+keywords[key[pre]]['word'].length, post[i]), "style": "color:none"};
		}
	//	console.log( key[i] + ' ' +kid);
		if (key[i] === kid) {
			ret[j++] = {'text':text.substring(post[i], post[i]+keywords[key[i]]['word'].length), 
				"style": "color:"+ color_k(key[i]) +"; font-weight: bolder "};		
		}else {
			ret[j++] = {'text':text.substring(post[i], post[i]+keywords[key[i]]['word'].length), 
				"style": "color:"+ color_k(key[i]) };
		}
		pre = i;
	}
	
		ret[j++] = {'text':text.substring(post[pre]+keywords[key[pre]]['word'].length, text.length), "style": "color:none"};		
